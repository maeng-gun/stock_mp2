...
ui <- dashboardPage(
  ...
  body = dashboardBody(
    ...
    tabItems(
      ...
      # 1) 개별주식 ####
      tabItem(tabName = "ind_stock",
        uiOutput('ind_stock')
      ),
      
      # 2) 복합주식 ####
      tabItem(tabName = "multi_stock",
      ),
      ...
      tabItem(
        ...
        box(
          ...
          div(
            actionButton(
              inputId = 'crawling-btn1',
              label = '수집'
            ),
            actionButton(
              inputId = 'crawling-btn2',
              label = '초기화'
            ),
            align='right'),
          br(),
...
server <- function(input, output, session) {
  ...
  wtr <- Waitress$new(theme = "overlay-percent")

  observe({
    if (input$menu_tabs == "ind_stock"){
      wtr$auto(value=5, ms=150)
      ind_stock_Server("ind_stock")
      wtr$close()
      output$ind_stock <- renderUI({
        ind_stock_UI("ind_stock")
      })
    }
  })

  observe({
    if(input$menu_tabs=='crawling'){
      library(glue)
      library(httr)
      source('utils_db.R')
      ...
      observeEvent(input$confirm1,{
        ...
        if(isTRUE(input$confirm1)){
          progressSweetAlert(
            id = "myprogress0",
            title = "주식데이터 수집 준비 중...",
            display_pct = TRUE, value = 0, total = 1
          )
          ...
          updateProgressBar(id = "myprogress0", value = 1, total=1)
          closeSweetAlert()
          
          if('workdays' %in% values$check){
            res_workdays <- map_dfr(values$years_2b_updated, 
                           ~get_workdays_from_krx(.x, save=input$save))
            output$workdays <- renderDT(res_workdays, options = list(scrollX = TRUE))
          }
          if('index_list' %in% values$check){
            res_index <- get_index_list(save=input$save)
            output$index_list <- renderDT(res_index, options = list(scrollX = TRUE))
          }
  ...
  observeEvent(input$endApp, {stopApp()})

}

# Run the application 
shinyApp(ui = ui, server = server,
         options = list(port=5987))
