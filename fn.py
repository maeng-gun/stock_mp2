...
def get_table_info(table=None, prep=None, numeric=False, item=None, new=False):
    all_list = pd.read_csv(os.getcwd()+'/util/fn_tables.csv')
...
def get_fiscal_basis_data(symbols, year_range, item=None, new=False):
    ...
    if item:
        item_name, item_kind, item_code, item_name_k, item_freq = get_table_info(item=item, new=new)
    else:
        item_name, item_kind, item_code, item_name_k, item_freq = get_table_info('fs', new=new)
...
def get_cross_section_data(symbols, fs=None, daily=None, item=None, new=False):
    ...
    if item:
        ...
        item_name, item_kind, item_code, item_name_k, item_freq = get_table_info(table, new=new)
    ...
    ft.close()
    col = get_table_info(table, numeric=True, new=new)
...
def get_time_series_data(date_range, symbols, new=False):
    ...
    item_name, item_kind, item_code, item_name_k, item_freq = get_table_info('daily', new=new)
    ...
    col = get_table_info('daily', numeric=True, new=new)
    df[col] = df[col].apply(pd.to_numeric, errors='coerce')
